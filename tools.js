  ...
  const tasks = Tasks([
      ...
      task: async () => {
        if (
          await util.promisify(fs.exists)(
            path.join(__dirname, 'plugin', 'backend'),
          )
        ) {
          await build();
        }
    ...
    },
    {
      ...
      task: async () => {
        if (
          await util.promisify(fs.exists)(
            path.join(__dirname, 'plugin', 'frontend'),
          )
        ) {
          await fse.copy(
            path.join(__dirname, 'plugin', 'frontend'),
...
const build = async () => {
  await spawn('npm', ['run', 'plugin:build'], { stdio: 'inherit' });
  await fse.remove(
    path.join(__dirname, 'dist', 'backend', 'tsconfig.backend.tsbuildinfo'),
  );
};
